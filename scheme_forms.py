...
def do_define_form(expressions, env):
    ...
    >>> # problem 10
    >>> env = create_global_frame()
    >>> do_define_form(r x 2))"), env) # evaluating (define (f x) (+ x 8))
    'f'
    >>> scheme_eval(read_line("(f 3)"), envead_line("((f x) (+)
    5
    """
    ...
    if scheme_symbolp(signature):
        ...
        bodi= expressions.rest
        lamb= do_lambda_form(Pair(formal, bodi), env)
        env.define(signature.first, lamb)
        return signature.first
    
        #do_define_form((define signature.first (lambda (signature.rest) (expressions.rest))), env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    inst= LambdaProcedure(formals, expressions.rest, env)
    return inst
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    elif not expressions.rest or expressions.first:
        return scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings:
        parfait=bindings.first
        validate_form(parfait, 2, 2)
        names=Pair(parfait.first, names)
        vals=Pair(scheme_eval(parfait.rest.first, env),vals)
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    muu= MuProcedure(formals, expressions.rest)
    return muu
    # END PROBLEM 11
